...
def test_put_note_does_not_change_flag_state(client_with_sources):
    """Writing a note no longer auto-changes assignment state."""
    import sqlite3
    ...
    row = conn.execute(
        "SELECT flagged FROM assignment WHERE source_id=? AND coder_id=?",
        (s1, coder_id),
    ).fetchone()
