"""Standalone import page for ACE."""
...
def _build(conn) -> None:
    ...
    with ui.column().classes("mx-auto q-pa-lg").style("max-width: 700px; width: 100%;"):
        ...
        if existing:
            ...
            ui.separator().classes("q-my-md full-width")
        ...
        ui.upload(
            ...
            auto_upload=True,
            on_upload=lambda e: _handle_file_upload(
                e, state, preview_container, column_container, import_container
            ),
        ...
        _build_import_ui(state, conn, column_container, import_container)


async def _handle_file_upload(
    e: events.UploadEventArguments, state, preview_container, column_container, import_container
):
...
def _handle_folder_load(
    folder_str, state, preview_container, column_container, import_container
):
...
def _build_import_ui(state, conn, column_container, import_container):
    ...
    with import_container:
        ...
        async def _do_import():
            ...
            if mode == "text_files":
                ...
                try:
                    ...
                    _show_result(count, folder.name, result_card, coding_button)
                except Exception as exc:
                ...
                try:
                    ...
                    _show_result(count, file_name, result_card, coding_button)
                except Exception as exc:
                    ui.notify(f"Import failed: {exc}", type="negative")
                    return
        ...
        coding_button = ui.button(
            "Start Coding \u2192",
            icon="arrow_forward",
            on_click=lambda: ui.navigate.to("/code"),
        ).props("unelevated color=primary").classes("q-mt-md")
        coding_button.set_visibility(False)


def _show_result(count: int, name: str, result_card, coding_button):
    """Show the import result summary and reveal the coding button."""
    result_card.clear()
    with result_card:
    ...
    coding_button.set_visibility(True)
